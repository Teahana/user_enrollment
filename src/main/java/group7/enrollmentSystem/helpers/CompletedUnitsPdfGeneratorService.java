...
@Service
public class CompletedUnitsPdfGeneratorService {
    ...
    public byte[] generateAuditPdf(StudentFullAuditDto auditDto) throws DocumentException, IOException {
        ...
        // Title
        document.add(new Paragraph("USP Completed Units Transcript", titleFont));
        document.add(new Paragraph("\n"));
        ...
        addAuditSection(document, "Failed:", filterCoursesByStatus(auditDto.getProgrammeCourses(), "Failed"), headerFont, normalFont);
    ...
    private List<CourseAuditDto> filterCoursesByStatus(List<CourseAuditDto> allCourses, String status) {
        return allCourses.stream()
                .filter(c -> {
                    ...
                    if ("Failed".equalsIgnoreCase(status)) return c.isFailed();
